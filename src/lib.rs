...
/// Calculate the length in samples for a delay. length ranges from 0.0 to 1.0.
fn delay(index: usize, mut length: f32) -> isize {
...
impl Default for PlexLooper {
    fn default() -> PlexLooper {
        PlexLooper::new(0.12, 0.66)
    }
}

impl PlexLooper {
    fn new(length: f32, dry_wet: f32) -> PlexLooper {
    ...
    /// Update the delay buffers with a new length value.
    fn resize(&mut self, n: f32) {
        let old_length = mem::replace(&mut self.length, n);

        for (i, buffer) in self.buffers.iter_mut().enumerate() {
            // Calculate the size difference between delays
            let old_delay = delay(i, old_length);
...
impl Plugin for PlexLooper {
    fn get_info(&self) -> Info {
        Info {
            name: "Plex Looper".to_string(),
            vendor: "Sun Machines".to_string(),
            unique_id: 243723076,
            version: 0002,
            inputs: 2,
            outputs: 2,
    ...
    fn get_parameter_text(&self, index: i32) -> String {
        match index {
            0 => format!("{}", (self.length * 10000.0) as isize),
